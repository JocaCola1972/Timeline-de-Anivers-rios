...
const AdminPanel: React.FC<Props> = ({ users, relationships, onAdd, onUpdate, onDelete, onUpdateRelationships }) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    const { sign, traits } = getZodiac(new Date(formState.birthdate!));
  ...
  const addTag = () => {
    if (tagInput.trim() && !formState.likes?.includes(tagInput.trim())) {
      setFormState(prev => ({ ...prev, likes: [...(prev.likes || []), tagInput.trim()] }));
      setTagInput('');
    }
  };

  const removeTag = (tag: string) => {
    setFormState(prev => ({ ...prev, likes: prev.likes?.filter(t => t !== tag) }));
  };
    ...
    <div className="space-y-6">
      ...
      <div className="flex border-b border-slate-200">
        <button
          ...
          className={`px-6 py-3 font-bold text-sm transition-all border-b-2 ${activeSubTab === 'users' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
        >
        ...
        <button
          onClick={() => setActiveSubTab('rels')}
          className={`px-6 py-3 font-bold text-sm transition-all border-b-2 ${activeSubTab === 'rels' ? 'border-indigo-600 text-indigo-600' : 'border-transparent text-slate-500 hover:text-slate-700'}`}
        >
          <div className="flex items-center gap-2">
            <LinkIcon className="w-4 h-4" /> Relações Globais
          </div>
        </button>
      </div>
      ...
      {activeSubTab === 'users' ? (
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
            <div>
              <h2 className="text-xl font-bold text-slate-800">Base de Dados de Utilizadores</h2>
              <p className="text-slate-500 text-xs">Total: {users.length} utilizadores registados.</p>
            </div>
            <button
        ...
        <div className="space-y-6 animate-in fade-in duration-300">
          <div className="bg-white rounded-2xl border border-slate-200 p-6 shadow-sm">
            <h3 className="font-bold text-slate-800 mb-4 flex items-center gap-2">
              <Plus className="w-5 h-5 text-indigo-500" /> Criar Novo Vínculo
            </h3>
            <form onSubmit={handleCreateRel} className="grid grid-cols-1 md:grid-cols-4 gap-4">
              <select 
                value={newRel.u1} 
                onChange={e => setNewRel({...newRel, u1: e.target.value})}
                className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-sm"
                required
              >
              ...
              <select 
                value={newRel.u2} 
                onChange={e => setNewRel({...newRel, u2: e.target.value})}
                className="px-4 py-2 bg-slate-50 border border-slate-200 rounded-xl outline-none text-sm"
                required
              >
              ...
              <button type="submit" className="bg-indigo-600 text-white font-bold rounded-xl hover:bg-indigo-700 transition-all text-sm py-2">
                Criar Relação
              </button>
            </form>
          </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="bg-slate-50 border-b border-slate-200">
                  ...
                  <th className="px-6 py-4 text-xs font-bold text-slate-500 uppercase text-center">Tipo</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {relationships.map(rel => {
                  const u1 = users.find(u => u.id === rel.userId);
                  const u2 = users.find(u => u.id === rel.relatedUserId);
                  return (
                    <tr key={rel.id} className="hover:bg-slate-50">
                      <td className="px-6 py-4 text-sm font-bold text-slate-700">{u1?.name}</td>
                      <td className="px-6 py-4 text-center">
                        <span className="px-2 py-1 bg-indigo-50 text-indigo-700 text-[10px] font-bold rounded-full uppercase">
                          {RELATIONSHIP_LABELS[rel.type]}
                        </span>
                      </td>
                      <td className="px-6 py-4 text-sm font-bold text-slate-700">{u2?.name}</td>
                      <td className="px-6 py-4 text-right">
                        <button onClick={() => deleteRel(rel.id)} className="text-red-400 hover:text-red-600">
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-white w-full max-w-2xl rounded-3xl shadow-2xl border border-slate-200 overflow-hidden">
            <div className="px-6 py-4 border-b border-slate-100 flex items-center justify-between bg-slate-50">
              <h3 className="text-lg font-bold text-slate-800">{editingUser ? 'Editar' : 'Novo'}</h3>
            ...
            <form onSubmit={handleSubmit} className="p-6 space-y-6">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                ...
                <input type="date" value={formState.birthdate} onChange={e => setFormState({...formState, birthdate: e.target.value})} className="px-4 py-2 bg-slate-50 border rounded-xl" required />
              </div>
              <button type="submit" className="w-full py-3 bg-indigo-600 text-white rounded-xl font-bold">Gravar</button>
            </form>
          </div>
        </div>
      )}
