...
import { Filter, Users, Shield, Calendar } from 'lucide-react';
  ...
  return (
    <div className="w-full bg-white rounded-xl shadow-sm border border-slate-200 p-6">
      ...
      <div className="relative overflow-x-auto pb-8 -mx-6 px-6 no-scrollbar">
        <div className="flex items-end min-w-max h-64 gap-8">
                  ...
                  {monthUsers.sort((a, b) => getDate(parseISO(a.birthdate)) - getDate(parseISO(b.birthdate))).map((user) => {
                    ...
                    return (
                      ...
                      >
                        {/* Birthday Pulse */}
                        {isBirthdayToday && (
                          <div className="absolute inset-0 -m-1 rounded-full bg-indigo-400 animate-ping opacity-75 motion-reduce:animate-none motion-reduce:scale-110 motion-reduce:opacity-20" />
                        )}

                        <div className={`relative z-10 w-12 h-12 rounded-full border-2 overflow-hidden bg-white ${isBirthdayToday ? 'border-indigo-500 scale-110' : 'border-slate-200'}`}>
                          {isPrivate ? (
                        ...
                        <AnimatePresence>
                          {hoveredUser === user.id && (
                            <motion.div
                              ...
                              className="absolute bottom-full left-1/2 -translate-x-1/2 mb-3 z-50 w-48 bg-white shadow-xl border border-slate-100 rounded-lg p-3 text-center"
                            >
                              ...
                              <div className="mt-2 pt-2 border-t border-slate-50 flex items-center justify-center gap-2">
                                <span className="text-[10px] px-2 py-0.5 bg-slate-100 rounded-full text-slate-600 font-bold uppercase">
                                  {user.zodiacSign}
                                </span>
                              </div>
                            </motion.div>
