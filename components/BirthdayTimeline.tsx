  ...
  return (
    <div className="space-y-8 pb-20">
      <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-200 p-8">
        ...
        <div className="flex overflow-x-auto gap-12 pb-6 no-scrollbar min-h-[320px] items-end px-2">
          {MONTHS_PT.map((name, i) => {
            ...
            return (
              <div key={name} className="flex flex-col border-l-2 border-slate-50 pl-6 min-w-[160px] last:pr-6 group">
                <span className="text-[11px] font-black text-slate-300 uppercase tracking-[0.2em] mb-8 group-hover:text-indigo-400 transition-colors">{name}</span>
                <div className="flex flex-wrap gap-5 items-end h-56">
                  {mUsers.sort((a, b) => getDate(new Date(a.birthdate)) - getDate(new Date(b.birthdate))).map(user => {
                    ...
                    return (
                      ...
                      >
                        {isToday && (
                          <div className="absolute inset-0 bg-indigo-400 rounded-full animate-ping opacity-30 scale-150" />
                        )}
                        
                        <div className={`w-16 h-16 rounded-[1.5rem] border-2 overflow-hidden bg-white shadow-lg transition-all duration-300 hover:scale-110 hover:shadow-indigo-100 ${isToday ? 'border-indigo-500 ring-4 ring-indigo-50' : 'border-slate-100 hover:border-indigo-200'}`}>
                          <img 
                            src={user.avatarUrl || `https://ui-avatars.com/api/?name=${encodeURIComponent(user.name)}`} 
                        ...
                        {user.wishlist && (
                          <div className="absolute -top-2 -right-2 w-6 h-6 bg-pink-500 rounded-lg border-2 border-white flex items-center justify-center shadow-md">
                            <Gift className="w-3 h-3 text-white" />
                          </div>
                        )}
                        
                        <div className="text-center mt-3">
                          <span className={`text-[11px] font-black transition-colors ${isToday ? 'text-indigo-600' : 'text-slate-400 group-hover:text-slate-600'}`}>
